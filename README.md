...
Созданы каркас репозитория, исследовательские read-only инструменты, engine-neutral модель собственного сценария v1, детерминированная JSON persistence и headless-основа редактора. Формат поддерживает terrain overrides и инертные object placements; внешний игровой адаптер преобразует их в детерминированные scene data без зависимости ядра от Godot. Игровые механики, графический редактор, импорт исходных форматов и Godot host ещё не реализованы.
